...
class FFFT_varform_2sites(VariationalForm):
    """
    ...
    CONFIGURATION = {
        'name': 'FFFT_varform_2sites',
        'description': 'FFFT_varform_2sites Variational Form',
        'input_schema': {
            '$schema': 'http://json-schema.org/schema#',
            'id': 'FFFT_varform_2sites_schema',
            'type': 'object',
            'properties': {
                'depth': {
                    'type': 'integer',
    ...
    def construct_circuit(self, parameters, q=None):
        ...
        for block in range(self._depth):
            ...
            if self._switch_V_T: # applying T than V
                ...
                circuit.append(F0_gate(), [q[i] for i in range(Nsite)])
                circuit.append(F0_gate(), [q[i+Nsite] for i in range(Nsite)])
                ...
                # exp(i theta V)
                circuit.append(expV(V_params*U/4, Nsite), [q[i] for i in range(Norb)])
